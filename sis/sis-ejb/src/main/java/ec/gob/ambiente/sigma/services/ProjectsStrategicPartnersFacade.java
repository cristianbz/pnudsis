...
public class ProjectsStrategicPartnersFacade extends AbstractFacade<ProjectsStrategicPartners, Integer> {
	...
	public List<ProjectsStrategicPartners> listaProyectosSocioEstrategico(String rucSocio)throws Exception{
		String sql="SELECT PSP FROM ProjectsStrategicPartners PSP,Projects P, Partners PA WHERE PSP.projects.projId = P.projId  AND PSP.partners.partId = PA.partId AND PA.partIdNumber=:rucSocio AND PSP.pspaStatus=TRUE AND P.projRegisterStatus='V' AND P.projStatus=TRUE ORDER BY P.projTitle";
		Map<String, Object> camposCondicion=new HashMap<String, Object>();
